...
export async function POST(req) {
  try {
    console.log(
      "[fourbiz-credentials] POST - Iniciando salvamento de credenciais"
    );

    const session = await getServerSession(authOptions);
    console.log(
      "[fourbiz-credentials] Session:",
    ...
    console.log(
      "[fourbiz-credentials] Senha recebida:",
      password ? "Sim" : "Não"
    );
    ...
    // Criptografa a senha do 4Biz usando AES-256-GCM
    // A senha pode ser descriptografada quando necessário para login automático
    console.log("[fourbiz-credentials] Criptografando senha...");
    const encryptedPassword = encrypt(password);
    console.log(
      "[fourbiz-credentials] Atualizando usuário:",
      session.user.email
    );
    ...
    const user = await User.findOneAndUpdate(
      ...
      {
        fourBizEmail: email,
        fourBizPassword: encryptedPassword,
