...
import {
  fetchTicketsFromAPI,
  organizarDadosTicket,
  extrairCookiesDeAutenticacao,
} from "./crawler.js";
...
export async function login4Biz(email, password, onProgress = null) {
  ...
  try {
    ...
    // Lança nova instância do browser
    browser = await launchBrowser();
    reportProgress(5, "Navegador iniciado");

    const pageData = await createPage(browser);
    page = pageData.page;
    ...
    reportProgress(92, "Buscando chamados via API...");

    // Extrai os cookies de autenticação necessários
    const { sessionCookie, authToken } =
      extrairCookiesDeAutenticacao(cookieString);

    // Busca tickets usando a API REST ao invés de web scraping
    ...
    const tickets = ticketsAPI.map(organizarDadosTicket);
